...
- Bannerlord 1.3.15 `NameGenerator` compatibility: `GenerateHeroFirstName(Hero)` is an instance method in the target game build. v1.7.4 binds the Harmony hook to the actual instance method instead of incorrectly searching only static methods.
...
Successful startup should report the battle-result patch, the promoted-troop naming patches, and the lord-promotion roster patch. On a Distinguished Service build without `get_using_extern_namelist()`, the optional external-list hook is skipped while the remaining promoted-name hooks continue to apply.
