...
Successful startup should report the battle-result patch, the promoted-troop identity patch, the promoted-troop naming patches, and the lord-promotion roster patch. On a Distinguished Service build without `get_using_extern_namelist()`, the optional external-list hook is skipped while the remaining promoted-name hooks continue to apply.
